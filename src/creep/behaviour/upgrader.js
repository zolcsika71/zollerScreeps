...
let behaviour = new Creep.Behaviour('upgrader');
...
behaviour.selectStrategies = function (actionName) {
    return [behaviour.strategies.defaultStrategy, behaviour.strategies[actionName]];
};
behaviour.invalidCreep = c => ['miner', 'upgrader'].includes(c.data.creepType) && c.data.determinatedSpot &&
    (c.data.ttl > c.data.spawningTime || c.data.ttl > c.data.predictedRenewal);
behaviour.approach = function(creep) {
    ...
    if( range > 0 ) {
        ...
        if (range === 1) {
            if (creepAtSpot.length && _.some(creepAtSpot, behaviour.invalidCreep)) {
