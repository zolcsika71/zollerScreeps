...
action.isValidTarget = function (target) {
    ...
    if (target.structureType === STRUCTURE_LINK) {
        ...
        return target.sum < ((target.source === true && target.controller === true) ? target.storeCapacity * global.MANAGED_CONTAINER_TRIGGER : target.storeCapacity);
    }
    return false;
};
...
action.work = function (creep) {
    ...
    if (creep.target.source === true && creep.target.controller === true) {
        ...
        let max = (creep.target.storeCapacity * global.MANAGED_CONTAINER_TRIGGER) - creep.target.sum;
